...
public class BibliotecaController {
	...
	public ResponseEntity<?> aggiornaCopiaLibro(@RequestBody LibroRequest libro, Long id) {
		String codice = libro.getCodice();
		Set<Copia> copie = new HashSet<>();
		Libro lib;
		libroRepository.existsByCodice(libro.getCodice());
	...
	public ResponseEntity<?> aggiungiCopiaLibro(@RequestBody LibroRequest libro) {
		...
		if (!existsByCodice(codice)) {
			...
			return ResponseEntity
					.badRequest()
					.body(new MessageResponse("Error: Il libro già esiste"));
		}
	}
